...
export default function Testimonials() {
  ...
  useEffect(() => {
    supabase
      .from('testimoni')
      .select('id, nama, angkatan, jabatan, isi, foto_url, inisial, warna_avatar, urutan')
      .eq('is_aktif', true)
  ...
  return (
    <section className="bg-white py-24 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {items.map(item => {
            const initials = item.inisial || getInitials(item.nama)
            const color = item.warna_avatar || '#1A5C38'
            return (
              <div
                key={item.id}
              ...
              >
                ...
                <div className="flex items-center gap-3 pt-5 border-t border-gray-100">
                  <div className="w-10 h-10 rounded-full flex-shrink-0 overflow-hidden">
                    {item.foto_url
                      ? <img src={item.foto_url} alt={item.nama} className="w-full h-full object-cover" loading="lazy" decoding="async" />
                  ...
                  <div>
                    <div className="text-[#0A2415] font-bold text-sm">{item.nama}</div>
                    {item.angkatan && <div className="text-gray-400 text-xs mt-0.5">{item.angkatan}</div>}
                    {item.jabatan && <div className="text-gray-400 text-xs mt-0.5">{item.jabatan}</div>}
                  </div>
                </div>
              </div>
            )
